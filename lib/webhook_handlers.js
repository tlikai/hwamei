module.exports = {
  incoming: function (headerStr, params) {
    if (params.payload) {
  ...
  sentry: function (headerStr, params) {
    let lines = []
    let datetime = formattedDateTime()
  ...
  gitlab: function (headerStr, params) {
    let url = ''
    let title = ''
    let lines = []
  ...
  github: function (headerStr, params) {
    let url = ''
    let title = ''
    let lines = []
  ...
  gogs: function (headerstr, params) {
    let url = ''
    let title = ''
    let lines = []
    ...
    if (header['x-gogs-event'] === 'push') {
      const commit = params.commits[0]
      url = commit.url
      title = `【Gogs】${params.pusher.username} 提交了代码到 ${params.repository.full_name}`
      lines.push(title)
      lines.push(`【COMMIT ID】${commit.id}`)
      lines.push(`【COMMIT LOG】${commit.message.replace(/[\r\n]/g, "")}`)
      lines.push(`【URL】${url}`)
    } else {
      url = 'https://gogs.io/docs/features/webhook.html'
      title = `【Gogs】不支持的事件类型：${params}`
      //lines.push(title)
      lines.push(`如果需要，自己实现一下吧！`)
      console.log(JSON.stringify(params))
    }
